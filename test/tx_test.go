...
// TestSubmitFinalizationRejectsUnknownCommitmentTx settles a boarded
// covenant UTXO while the batch handler overrides the commitment tx with a
// locally-built PSBT that arkd never produced: the emulator must refuse to
// sign it, so the batch finalization fails.
func TestSubmitFinalizationRejectsUnknownCommitmentTx(t *testing.T) {
	...
	// a commitment tx arkd never built: any valid PSBT whose txid is not
	// indexed by arkd
	bogusTx := wire.NewMsgTx(2)
	bogusTx.AddTxIn(wire.NewTxIn(&wire.OutPoint{Index: 0}, nil, nil))
	bogusTx.AddTxOut(&wire.TxOut{Value: contractOutput.Value, PkScript: alicePkScript})
	bogusPtx, err := psbt.NewFromUnsignedTx(bogusTx)
	require.NoError(t, err)
	bogusCommitmentTx, err := bogusPtx.B64Encode()
	require.NoError(t, err)
	...
	emulatorBatchHandler := &boardingBatchEventsHandler{
		delegateBatchEventsHandler: &delegateBatchEventsHandler{
			...
			OverrideCommitmentTx: bogusCommitmentTx,
		},
		boardingVtxo: vtxo,
	}

	topics := arksdk.GetEventStreamTopics(
		[]types.Outpoint{vtxo.Outpoint}, []tree.SignerSession{treeSignerSession},
	)
	eventStream, stop, err := grpcClient.GetEventStream(ctx, topics)
	require.NoError(t, err)
	t.Cleanup(func() {
	...
	require.Empty(t, commitmentTxid)
}
